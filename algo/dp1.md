...
- 下标为负时怎么处理、char 转 int 以便处理：见 [3320. 统计能获胜的出招序列数](https://leetcode.cn/problems/count-the-number-of-winning-sequences/description/) 的代码
- 数组中选两个不相交的子集，可以设计状态为 `f[i][j][k]` 表示前 `i` 个数，第一个子序列的状态为 `j`，第二个子序列的状态为 `k` 的方案数，然后考虑第 `i` 个数：不放、放第一个、放第二个的转移
- 给一个由 `a`、`b`、`c` 构成的字符串，使得字符串不含 `abc` 子序列最少删除多少个字符？这个问题可以用状态机来思考，`f[i][0/1/2]` 表示 **没有a**，**有a没有ab** 和 **有ab没有abc**，答案为 `min(f[n])`
    - 进阶：如果每次修改一个字符，如何快速求出每次修改后的答案？单点修改->线段树，维护 **a：区间有多少个 a。b：区间有多少个 b。c：区间有多少个 c。ab：要使区间没有子序列 ab，至少要操作多少次。bc：要使区间没有子序列 bc，至少要操作多少次。abc：要使区间没有子序列 abc，至少要操作多少次。** 例题为 [CF1609E](https://codeforces.com/problemset/problem/1609/E)
- 粉刷房子变形题：除了相邻房子颜色不相等，还要求对称位置颜色不相等（即 $c[i] \neq c[n-i-1]$）。加上这个限制之后怎么做？**改变枚举顺序**，按照 $0,n-1,1,n-2,...$ 这样的顺序枚举，这样第 $i$ 个与第 $i-2$ 个相邻，且 $i$ 为奇数时，$i$ 与 $i-1$ 颜色不能相等。因此维护 $f[i][j][k]$ 表示第 $i$ 个颜色为 $j$，第 $i-1$ 个颜色为 $k$ 的集合即可。这里的初始化是 $f[1][*][*]$，然后从 $i=2$ 开始枚举。题目见 [LC3429](https://leetcode.cn/problems/paint-house-iv/description/)
- 树形 DP 题，包含反转子树权值操作的话，就要考虑维护子树的最大和以及最小和。如果题目中还限制翻转一个点后，往下 $k$ 级儿子才可以翻转，就要多设计一个状态表示「最近翻转祖先的距离」或者「当前还需往下多少步才可以翻转」。如果维护的状态表示「距离**至少**为 $i$」，那么计算答案时要取后缀最值。例题参考 [LC3544](https://leetcode.cn/problems/subtree-inversion-sum/description/)
- 划分型 DP 中，需要保证子数组的极差不超过 $k$，枚举右端点时，怎么确定左端点呢？**方法一：**用一个 `multiset` 维护区间内的元素，只要 `*prev(s.end())-*s.begin()>k` 就移动左端点。**方法二：**用单调队列维护滑动窗口的最值，此法的复杂度更优
...
对于节点 $x$ 和一条边 $x-y$，考虑选或不选

- 不选，则转化为对于节点 $y$ 及其儿子，最多选 $k$ 条
- 选，则转化为对于节点 $y$ 及其儿子，最多选 $k-1$ 条

一开始我们可以都不选，然后再选出可以**最大化增量**的那些边，怎么求出来呢？

把选的减去不选的差值（$f(v,1)-f(v,0)$）放入数组中排序，然后选择前 $k$ 个大于 0 的值即可
...
- 要么加一个字符
- 要么删一个字符
- 要么把字符变成字母表的下一个（`z` 不能变）

本题的特点在于需要枚举最后出现的次数 `k`，对每个 `k` 进行一次 $O(\Sigma)$ 的 DP

`k` 的范围是 $1$ 到 $n$，因为保底是都添加到 $n$ 个

对于第 $i$ 种字符，要么不保留，全删；要么弄成 $k$ 个

怎么弄？要么添加，要么看看能否从第 $i-1$ 种变过来。注意，一个字符变两次不如一删一增，因此仅需考虑上一种即可
