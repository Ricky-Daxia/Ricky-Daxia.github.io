...
给一棵树，点权为 $0$，每次询问会让 $m$ 个点变成 $1$，每次操作把一个点及其子树全反转，回答最小操作次数

这道题的特点在于，$n,q, \Sigma(m)$ 范围都是 $1e5$，不能暴力做

如果 get 到了解法会觉得挺简单的，说白了就是

- 一个点是 $1$，需要对它和所有儿子都操作一次
- 但是这样会多算，如果发现一个 $1$ 的父节点也是 $1$ 时就要减去一次

因此可以总结出下面的写法，复杂度是 $O(\Sigma(m))$ 的
...
### 每次相邻两数 +1 或 -1，能否使数组变为 0

题源 [ARC119C](https://atcoder.jp/contests/arc119/tasks/arc119_c) 

给定数组 a，每次操作，你可以选择两个相邻的数字，把它们都加一，或者都减一。
...
$n=3$ 时，$a,b,c$ 需要满足 $b=a+c$

$n=4$ 时，需要把两边消掉，变成 $0,b-a,c-d,0$，转化为 $a+c=b+d$

$n=5$ 时，变成 $0,b-a,c,d-e,0$，转化为 $a+c+e=b+d$

这样就发现了性质：合法数组的**奇数位置**的和等于**偶数位置**的和。如果把其中一种位置和变成负的，就转化为数组和为 $0$，经典的前缀和问题
