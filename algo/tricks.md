...
在 [LC3568](https://leetcode.cn/problems/minimum-moves-to-clean-the-classroom/description/) 这道题中，需要在 BFS 时维护当前的「可使用步数」和「已收集目标」，把这些维度加入到 `vis` 数组中记录
...
去重技巧：做一个规定，插入在 $s[i]$ **左侧**的字符，不能和 $s[i]$ 相同，这不会影响答案的正确性

枚举最后一个字符的右侧插入了多少个字符，设为 $i$，这些字符没有限制，有 $26^i$ 种方案

剩下 $(n-1) + (k-i)$ 个字符，我们需要考虑其中 $n-1$ 个字符的位置，这就是 $C(n-1+k-i, n-1)$

因此方案数就是 $\displaystyle \sum_{i=0}^{k} 26^{i}* \binom{c-1+k-i}{n-1} * 25^{k-i}$

### 构造排列使得 gcd(i,a[i]) 之和为 k

$k$ 的范围为 $[n,\dfrac{n(n+1)}{2}]$ 时有解
...
给数组，问满足条件的下标对数 $(p,q,r,s)$，将 $a\cdot c=b\cdot d$ 变形为 $\dfrac{a}{b}=\dfrac{d}{c}$，这样可以前后缀分解

怎么维护这个分数呢？将 $\dfrac{a}{b}$ 变为**最简分数**，做法是同除它们的 GCD，然后插入哈希表中。如何插入？分子分母映射为一个 key 值即可
...
定理：如果有 $diff$ 个位置与目标不同，需要交换 $\dfrac{diff}{2}$ 次即可

首先因为 $\dfrac{diff}{2}$ 是下限，然后证明可以用这么多次交换做到
...
拆位计算，假设有 $x$ 个数满足第 $i$ 位为 $1$，那么就对答案贡献 $2^i *x*(n-x)$
...
给定 $a$ 和 $b$，每次操作选定任意 $x$，令 $a= \lfloor \dfrac{a+x}{2}\rfloor$，$b=\lfloor \dfrac{b+x}{2}\rfloor$，问最少几次操作可以使 $a=b$？
