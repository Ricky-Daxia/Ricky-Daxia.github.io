...
- 基本方案：`hash(key)%n`，但问题是 `n` 在变化怎么办
- 一致性哈希：将 server 标识与 key 映射到同一个值域空间，也就是把 server 和 key 都做一个 hash，形成一个钟表的形状，key 落位后顺时针找到最近的 server 映射点
  - ![](https://cdn.jsdelivr.net/gh/Ricky-Daxia/Hei_Xiu/202408012315749.png)
...
微服务的缺点：网线传输是不可靠的，存在调用雪崩现象，这时就需要**断路器**
...
- pub-sub 模式：publisher 根据 topic 通知 subscribers（中间有个 channel）
  - ![](https://cdn.jsdelivr.net/gh/Ricky-Daxia/Hei_Xiu/202408012328609.png)
